...
> [!WARNING]
> 当前版本只在“没有其他 Codex 任务同时运行”的单任务场景完成真人验收。实测发现，Codex Desktop 在其他任务同时运行时可能发生实时语音会话状态或事件路由冲突，表现为圆球仍在工作但长语音没有落入当前任务。本项目无法从外部修复 Codex 客户端内部的多任务路由；多任务并行时请不要使用本工具，并等待客户端官方修复后重新验证。
...
- 仅验证单任务场景；其他 Codex 任务同时运行时，客户端可能发生实时语音状态或事件路由冲突。本项目不修改客户端，因此无法保证多任务场景可用。
